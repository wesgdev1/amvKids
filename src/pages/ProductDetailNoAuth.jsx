...
import { useModelNoAuth } from "../domain/models/useModelNoAuth";
...
export const ProductDetailNoAuth = () => {
  const params = useParams();
  const { id } = params;
  const { user } = useContext(AuthContext);
  if (user) {
    window.location.href = `/productos/${id}`; // Redirigir a la página de detalle del producto con autenticación
  }

  const { data, loading, error } = useModelNoAuth(id);
